obs<-bdgraph.sim(p=16,graph="random",n=1000,type="Gaussian",vis=TRUE)
g0<-obs$G
dd<-obs$data
...
col = c( "binary" = "gold")
ggnet2(net1,node.size=5,color="steelblue",label = TRUE)


NS<-bdgraph.mpl(data=dd,method="ggm",algorithm="bdmcmc",iter=5000,burnin=2000,save.all=TRUE)
summary(NS)
...
TP1<-length(intersect(a,b1))/allt
FP1<- length(setdiff(b1,a))/allf
